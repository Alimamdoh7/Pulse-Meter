...
extern volatile uint32 Capture_Flag_g      ;   // For ICU with UltraSonic Sensor
...
ISR(TIMER1_CAPT_vect)
{
	...
	else if (Capture_Flag_g == 2)
	{
		...
		#if (ICU_MODE == CAPTURE_FULL_TIME)
		//Detect Rising edge
		TCCR1B |= 0X40;
		#elif (ICU_MODE == CAPTURE_ON_TIME)
        //DISABLE ICU INTERRUPT
        CLR_BIT (TIMSK , 5);
		#endif //#if (ICU_MODE == CAPTURE_FULL_TIME)
	}
	#if (ICU_MODE == CAPTURE_FULL_TIME)
